...
from observability.openlineage_emitter import emit_openlineage_events
...
def main() -> None:
    ...
    # 2. Automatic Quarantine Evaluation (Bonus)
    clean_orders, quarantined_orders, quarantine_summary = quarantine_records(orders, orders_contract)

    # 3. Soda Core Data Contract Validation (Bonus)
    soda_contract_path = ROOT / "contracts" / "soda_orders_contract.yml"
    soda_issues = []
    if soda_contract_path.exists():
        soda_contract = load_soda_contract(soda_contract_path)
        soda_issues = validate_soda_contract(orders, soda_contract)

    # 4. Anomaly Detection on Order Volume (with Seasonality MAD / same-weekday)
    ...
    # 6. Knowledge Base Validation & Freshness
    ...
    # 7. RAG Observability Signals (Token shift, embedding norm, cosine drift, token frequency drift)
    current_texts = [d["content"] for d in docs]
    text_result = detect_text_length_shift(
        current_texts, history["mean_text_length"].tail(14).tolist()
    )
    ...
    cosine_drift_res = detect_embedding_cosine_drift(
        current_embeddings=[[0.1, 0.9, 0.4], [0.12, 0.88, 0.41]],
        baseline_centroid=[0.11, 0.89, 0.40],
    )
    token_drift_res = detect_token_frequency_drift(
        current_texts=current_texts,
        baseline_texts=current_texts,
    )

    # 8. SLO Calculations & Multi-window Multi-Burn-Rate Policy
    ...
    # 10. OpenLineage & Elementary OSS Report Generation
    emit_openlineage_events()
    generate_elementary_observability_report(
        orders_df=orders,
        metrics_history_df=history,
    )
    ...
    report = {
        ...
        "soda_failed_checks": len([i for i in soda_issues if not i.get("passed", False)]),
        ...
        "kb_cosine_drift_signal": cosine_drift_res,
        "kb_token_drift_signal": token_drift_res,
    ...
    print("=== DATA RELIABILITY OBSERVABILITY BASELINE (WITH BONUS SUITE) ===")
    print(f"Orders rows              : {len(orders)} (Clean: {quarantine_summary['clean_rows']}, Quarantined: {quarantine_summary['quarantined_rows']})")
    print(f"Orders failed checks     : {len(orders_failed)} (critical: {len(orders_critical)}, action: {orders_action})")
    print(f"Soda Contract Checks     : {len(soda_issues)} checks evaluated")
    ...
    print(f"KB cosine drift          : {cosine_drift_res['is_anomaly']} (score={cosine_drift_res['score']:.4f})")
    ...
    print(f"OpenLineage Events       : Emitted to reports/openlineage_events.json")
    print(f"Elementary Report        : Generated at reports/elementary_report.json")
